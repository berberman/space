...
I figured it would be an excellent exercise to manually derive in Lean how this special eliminator, combined with {lean empty}`HEq`, implies K.
...
> Note: Because Lean inherently supports Uniqueness of Identity Proofs (UIP), we must be extremely careful not to rely on it accidentally.
  This means we cannot use the cases tactic, nor can we use Lean's built-in {lean empty}`Eq` (`=`).
  Every operation must be implemented manually by invoking the rec function generated by our custom inductive types.

We start by defining a homogeneous equality with a universe level of at least {lean empty}`Type`:
...
For heterogeneous equality, we will rely directly on Lean's built-in {lean empty}`HEq` (`≍`).
As long as we avoid functions like {lean empty}`eq_of_heq` and refrain from using `cases` for dependent pattern matching, we are safe.
...
We are only one step away! Obviously, the missing link can be filled by {lean empty}`eq_of_heq`, but remember: we are strictly restricted to using recursors.
...
we successfully eliminate from {lean empty}`HEq` to {lean empty}`Eq`. Now we can close out the proof:
...
Ultimately, the core contribution of this special eliminator is essentially {lean empty}`eq_of_heq` (known as `JMeq_eq` in Rocq), which we've just shown implies Axiom K.
...
It appears Lean inserted an {lean empty}`Eq` proof relying on UIP directly into the motive's parameters,
...
As we can see, the core of the automated proof uses {lean empty}`eq_of_heq` to convert {lean empty}`HEq` to {lean empty}`Eq`, and then eliminates it using {lean empty}`@Eq.rec`.
