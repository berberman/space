...
I figured it would be an excellent exercise to manually derive in Lean how this special eliminator, combined with `HEq`, implies K.
...
For heterogeneous equality, we will rely directly on Lean's built-in `HEq` (`≍`).
...
We need a way to connect it to `HEq` so we can exploit that special eliminator.
...
We can prove this using the universe-polymorphic `PSigma` (denoted here with `Σ'`).
...
we successfully eliminate from `HEq` to `Eq`. Now we can close out the proof:
...
It appears Lean inserted an `Eq` proof relying on UIP directly into the motive's parameters,
...
As we can see, the core of the automated proof uses `eq_of_heq` to convert `HEq` to `Eq`, and then eliminates it using `Eq.rec`.
