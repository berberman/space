...
Beyond the styling, I also implemented several useful feaures.
Any frontend code involved (CSS and JavaScript) was written by an LLM and unchecked. The visually apparent changes include:

1. Automatic Verso `htmlId` generation for every heading below `H2`.
...
2. A sidebar on the right that uses those anchors to show the section currently in view, much like a table of contents.
   Clicking a section title takes you there.

3. A reading-progress indicator in the bottom right corner that displays a percentage.
   Clicking it returns you to the top of the page.

4. An estimated reading time, shown as `xx min read`, after each post's date and category.
...
5. I found Verso's footnotes unintuitive. They are now rendered as numbered references,
   and a foonote list is generated at the bottom of each post, similar to Markdown, with backlinks to the original references.

6. Tables! While Verso's manual genere supports table rendering, blog genre doesn't.
...
7. I replaced utteranc.es with giscus and added styling consistent with the site's visual design.

# Feeds and SEO

I also added a few less visible features:
...
Think about it more carefully, Verso's whole model is pretty wile.
