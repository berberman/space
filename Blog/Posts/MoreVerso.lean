...
Thanks to LLMs, I was able to figure out the styling despite have no frontend skill.
With more fine-grained CSS control and overriding Verso's built-in CSS, I added syntax highlighting for both Lean and Prism.js code blocks that fit the rest of the site.

I also improved the layout of the Post page (formally called Archive): I adjusted the heading hierarchy, placed categories horizontally,
...
Previously, anchors had to be added manually whenever I wanted to reference a particular heading.
...
Currently the estimate is very rough: it counts all words, including those inside code block, and divides the total by 225 words per minute.

## Footnotes

I found Verso's footnotes[^weird] unintuitive. They are now rendered as numbered references, and a footnote list is generated at the bottom of each post, similar to Markdown, with backlinks to the original references.

[^weird]: I didn't even find the documentation!

## Tables

While Verso's manual genre supports table rendering, blog genre doesn't.
...
The idea here is to use Lean parser combinators to parse a row block dedicated to tables, while recording the source information for every cell.
Those positions need to be retained because a table may contain inline Lean code that still needs to be interpreted, and that code may depend on source locations, e.g. diagnostics.
...
Those contents get passed to Verso's inline elaborator and then wrapped in block paragraphs. At that point, table parsing and elaboration are complete, producing `Array (Array Term)`, where `Term` is our familiar Lean syntax.

The table is then packaged into a custom Verso block extension called `blogTable`, which is ultimately rendered as HTML in runtime.
